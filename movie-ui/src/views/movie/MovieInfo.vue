...
<el-table-column label="操作" width="240">
    ...
    <el-dialog title="添加电影" :visible.sync="addDialogVisible" width="60%" @close="addDialogClosed">
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px">
        ...
        <el-form-item label="累计票房/万" prop="movieBoxOffice">
          <el-input v-model="addForm.movieBoxOffice" placeholder="请输入票房数字(万)"></el-input>
        </el-form-item>
    ...
    <el-dialog title="修改电影信息" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="累计票房/万" prop="movieBoxOffice">
          <el-input v-model="editForm.movieBoxOffice" placeholder="请输入票房数字(万)"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 修改前检查是否有未完成场次 - 方案三：友好提示
    // 修改 isAbleEdit 方法
async isAbleEdit(id) {
  this.checkAbleId.movieId = id
  // 先获取电影名称用于提示
  const movieInfo = await axios.get('sysMovie/find/' + id)
  const movieName = movieInfo.data.data.movieName
  ...
  await axios.get('sysSession/isAbleEdit', {params: this.checkAbleId}).then(response => {
    ...
    if (response.data.total === 0) {
      ...
      for (let temp of sessions) {
        console.log(temp)
        // 简化显示：只显示场次日期、时间和编号
        sessionInfo += `\n  📅 ${temp.sessionDate} ${temp.playTime} (场次编号:${temp.sessionId})`
      }
      console.log('sessionInfo is : '+sessionInfo)
      this.$alert(
        `《${movieName}》有未完成的电影场次，不能修改电影信息。\n` +
        `以下场次尚未完成：${sessionInfo}`, 
        '修改请求异常通知', {
    ...
    async isAbleMultipleDelete() {
      ...
      for (let i = 0; i < this.multipleSelection.length; i++) {
        const movie = this.multipleSelection[i]
        this.checkAbleId.movieId = movie.movieId
      ...
      if (ableDelete === false) {
        let alertMessage = '以下电影有未完成场次，不能批量删除：\n'
        for (let movie of movieInfoList) {
          alertMessage += `\n《${movie.movieName}》的未完成场次：`
          for (let session of movie.sessions) {
            alertMessage += `\n  🎬 ${session.sysHall ? session.sysHall.hallName : '未知影厅'} - ${session.sessionDate} ${session.playTime} (编号:${session.sessionId})`
          }
        }
        
        this.$alert(alertMessage, '批量删除请求异常通知', {
          confirmButtonText: '我知道了',
          dangerouslyUseHTMLString: true,
          callback: action => {
            this.$router.push('/movie')
