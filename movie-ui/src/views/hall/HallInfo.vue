  ...
  methods: {
    ...
    async multipleDelete(){
      ...
      await axios.post('sysHall/delete', JSON.stringify(ids)).then(resp => {
        // 【关键修复】判断是否等于200
        if (resp.data.code !== 200){
          // 失败了用红色弹窗，并且 return 结束运行
          return this.$message.error('批量删除影厅分类失败！')
        } else {
          // 成功了才刷新列表并弹窗
          this.getHallList()
...
async deleteHallById(id){
      ...
      await axios.post('sysHall/delete', JSON.stringify(deleteInfo)).then(resp => {
        // 【关键修复2】判断后端返回的 code
        if (resp.data.code !== 200){
          // 失败了就用 error 红色弹窗，并且加了 return 阻止代码往下乱跑
          return _this.$message.error('删除影厅信息失败！') 
        } else {
          // 只有真正成功了，才刷新列表并弹出成功提示
          _this.getHallList()
          _this.$message.success('删除影厅信息成功！')
        }
      }).catch(err => {
         _this.$message.error('网络请求异常，请稍后再试！')
      })
    },
